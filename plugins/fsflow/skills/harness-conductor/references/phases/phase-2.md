...
怎么改由开发工程师根据源码和知识库证据自行决定；调用关系不明或跨域影响时用源码搜索和按需 LSP 查引用，graphify 可选；已有同版本同范围证据（含相关工作区变更）可复用。
...
开发交付说明按任务落盘到 `development-notes/<taskId>.md`，记录源码版本、调用方证据、回归项、自测结果和未确认范围。并行开发只更新各自任务文件，返修后更新说明。
后续审查、测试与交付的上下文会引用当前任务已有的说明；旧任务缺失说明时从源码查证，不增加文件存在性门控。
