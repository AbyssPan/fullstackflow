---
name: "kb-query"
description: "渐进式分层知识库检索。三层检索：L1 overview关键词匹配定位域 → L2 meta.yaml精确筛选 → L3 按需加载文档。支持4种模式：需求拆解/技术方案/接口搜索/知识问答。自动触发：代码修改、需求分析、接口查找、技术方案、改bug、新增功能等场景。前后端支持精确定位；源码搜索与按需 LSP 验证调用方和影响面。"
---
...
默认采用 `kb-query → 源码验证 → 按需查引用`。知识库负责业务定位、规范与历史经验；当前实现和依赖关系以源码为准。
...
实现事实冲突以源码为准，编码规范以知识库为准；疑似过期内容记录证据，交 Phase 5 提交前经授权更新。

---

## 分支与维护状态

知识库与代码同仓，始终使用当前分支的 `.docs/llm-knowledge/`。维护模式下索引的扫描提交与 HEAD 不符时，精确查询会放弃缓存关系并查询当前 Git 文件；未提交变更仍需核实源码。查询返回 knowledgeStatus，deferred 必须告知原因；updated 只表示曾完成审查，使用相关规则前仍应核对来源。按需读取域 specs/decisions 与 common/wiki 相关章节，识别生效、被替代和退役状态，不把历史规则当作当前规则。
