 ...
 *
 * 用法:
 *   node plugins/fullstackflow/scripts/commands/advance-phase.js <storyId> <phase>
 *     推进到指定 Phase（必须等于 currentPhase + 1）
 *   node plugins/fullstackflow/scripts/commands/advance-phase.js <storyId> 2 --renew-pass
 *     Phase 2 续签 dev-pass，不推进相位
 *   node plugins/fullstackflow/scripts/commands/advance-phase.js <storyId> 3 --lint-fix
 *     Phase 2→3 时按 task-dag.json 涉及的仓库逐个执行 eslint --fix
 *   node plugins/fullstackflow/scripts/commands/advance-phase.js <storyId> <phase> --auto-fix
 *     门控失败时先让 policy.js 尝试自动恢复，再重跑一遍门控
 *   node plugins/fullstackflow/scripts/commands/advance-phase.js <storyId> <phase> --rollback
 *     回退到更早 Phase：归档中间产出物；targetPhase <= 2 时重置修复预算
 *   node plugins/fullstackflow/scripts/commands/advance-phase.js <storyId> 2 --fix-loop
...
if (fixLoopFlag) {
  ...
  // 10. 构造 spawnPrompt（主 Agent 直接注入给全栈开发工程师）
  ...
  console.log(JSON.stringify({
    ...
    spawnPrompt,
    nextSteps: [
      `1. 主 Agent 将上述 spawnPrompt 作为 Prompt Spawn 全栈开发工程师(agent 注册名: fullstack-developer)`,
