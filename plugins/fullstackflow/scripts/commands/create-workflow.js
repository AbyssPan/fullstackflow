 ...
 *
 * 用法:
 *   node plugins/fullstackflow/scripts/commands/create-workflow.js <storyId> "<title>" [--bypass] [--figma] [--mode=run|fixbugs] [--input <file>]
 *   node plugins/fullstackflow/scripts/commands/create-workflow.js <storyId> --refresh-input [--figma]
 ...
 *
 * 使用场景:
 *   - 用户启动 FullstackFlow 后，由 harness-workflow.js start 内部调用 createWorkflow 创建状态文件；
 *     本命令是该调用的等价手工入口
 ...
 *   - 不走标准激活流程、需要单独为一个 Story 建流时，主 Agent 或用户手动执行本命令
 *
 * 说明:
