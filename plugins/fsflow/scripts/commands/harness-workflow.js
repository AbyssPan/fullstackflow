 ...
 *
 * 用法:
 *   node plugins/fsflow/scripts/commands/harness-workflow.js start <storyId> "<标题>" [--mode=run|fixbugs] [--input <file>]
 *     激活 harness 模式（写 .harness-active 标记文件）
 *   node plugins/fsflow/scripts/commands/harness-workflow.js end
 *     关闭 harness 模式（删除标记文件）
 *   node plugins/fsflow/scripts/commands/harness-workflow.js status
 *     查看当前状态
 ...
 *
 * 使用场景:
 *   - 用户执行 `/fsflow:run` 或 `/fsflow:fixbugs` 时主 Agent 执行 start：
 *     开启 dev-pass 强制校验（此后编辑 src/ 需持 pass），
 *     同时建好工作流状态文件
 *   - harness-start skill 的两步初始化：先写 story-input.json，再 start --input 一步建流
 *   - 工作流走到 Phase 7 完成、归档收尾后，用户执行 `/fsflow:end`，主 Agent 调用 end 删除标记，
 *     解除编辑限制
...
function cmdStart(storyId, title, mode, opts = {}) {
  ...
  if (existing && existing.active) {
    console.log(JSON.stringify({
      ok: false,
      message: `⚠️ Harness 模式已激活 (storyId: ${existing.storyId} "${existing.title}")，请先执行 /fsflow:end 结束当前工作流`,
      current: existing
    }))
    process.exit(0)
  }
...
switch (command) {
  ...
  default:
    console.log([
      ...
      '',
      '用户命令: /fsflow:run、/fsflow:fixbugs、/fsflow:status、/fsflow:end'
    ].join('\n'))
    process.exit(1)
}
