...
Agent 注册名 **`release-assistant`**（发布助手）。调用 `use_skill("fullstackflow:kb-update")` 增量更新
...
| 调用成功才算完成 | `use_skill("fullstackflow:kb-update")` 返回失败时不要把本 Phase 报成完成，门控看不出来，但 Phase 7 之后没人会回来补 |
