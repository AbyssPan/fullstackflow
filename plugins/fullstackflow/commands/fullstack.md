...
# /fullstack — FullstackFlow 端到端工作流
...
| 用法 | 说明 | 实际执行 |
|---|---|---|
| `/fullstack run <storyId> "<标题>"` | 新功能 / 页面级改造（有原型 / Figma 门控） | `use_skill("harness-start")` → mode=run |
| `/fullstack fixbugs <storyId> "<标题>"` | 缺陷修复（免原型文档，Phase 0 自动拉 TAPD 缺陷） | `use_skill("harness-start")` → mode=fixbugs |
| `/fullstack status [storyId]` | 查看工作流状态 | `node ${CLAUDE_PLUGIN_ROOT}/scripts/commands/harness-workflow.js status` |
| `/fullstack evolve [storyId]` | 自进化体检（audit → 度量 → 诊断 → 治疗 → 验证） | `use_skill("harness-evolve")` |
| `/fullstack archive <storyId>` | 归档 / 复档 / 归档历史 | `use_skill("harness-archive")` |

无参数直接执行 `/fullstack` 时：视为 `/fullstack run` 入口，按 harness-start 的
