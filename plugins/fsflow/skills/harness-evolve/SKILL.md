...
```
/fsflow:evolve <storyId>               # 完整五步闭环分析
/fsflow:evolve all                     # 分析所有已归档 Story
/fsflow:evolve <storyId> --check-only  # 只跑体检+度量
/fsflow:evolve <storyId> --propose-only # 只跑提案+验证
```
...
```bash
/fsflow:evolve STORY-20260710-01
/fsflow:evolve all --check-only
```
