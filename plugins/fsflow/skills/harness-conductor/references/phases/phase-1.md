...
Agent 注册名 **`task-planner`**（任务规划师，规格追踪驱动）。基于 Phase 0 产出物
（Capability + Requirement + Scenario 已内嵌在 `acceptance-criteria.json`）把需求拆成可并行的任务 DAG
（覆盖前端 / 后端 / DB 变更，拆解顺序：DB → 后端 → 前端 → 集成），并通过 AC 引用形成规格到任务的追踪链。
在 `task-dag.json` 的 `files[]` 中**列全所有待修改文件**（mapper XML、DB 脚本、配置文件均不可漏）——
该字段决定 Phase 2 的 dev-pass 写入范围，漏写的文件在 Phase 2 会被 hook 直接拒绝编辑。

有 Figma 链接时，本 Phase 才处理设计稿（需求分析师只标 UI 改动点、不拉稿）：
`use_skill("fsflow:figma-to-component-map")` 针对要拆的组件精确拉取，产出 frame 清单并给 task 绑
...
|------|------|-----|---------|------|
| **解析指引**（软） | 只看 `sources.figmaUrls` 非空 | ✅ 注入 | ✅ 注入 | prompt 里点名 `use_skill("fsflow:figma-to-component-map")`，禁止凭链接猜 UI 结构 |
