#!/usr/bin/env node
/**
 * validate-openspec.js — 内置 OpenSpec 规格产物校验（零依赖，规则提炼自 Fission-AI/OpenSpec）
 *
 * 校验 story 目录下 openspec/ 规格产物的结构合法性，替代 `openspec validate` CLI：
...
function main () {
  ...
  if (!storyId) {
    console.log(JSON.stringify({
      ...
      hint: '校验 story 目录下 openspec/ 规格产物的结构合法性（规则提炼自 OpenSpec，零外部依赖）'
    }, null, 2))
    process.exit(1)
  }
  let storyDir
