 ...
 *   独立执行:
 *     node plugins/fsflow/scripts/lib/trace.js agent-spawn  <storyId> <agentName> [taskId] [phase]
 *     node plugins/fsflow/scripts/lib/trace.js agent-result <storyId> <agentName> <completed|failed> [detailsJSON]
 *     node plugins/fsflow/scripts/lib/trace.js git          <storyId> <init|add|commit|push|mr> [success|failed] [detailsJSON]
 *     node plugins/fsflow/scripts/lib/trace.js phase-outcome <storyId> <phase> <outcome> [detailsJSON]
 *     不带子命令时打印 usage JSON
 *
 * 使用场景:
